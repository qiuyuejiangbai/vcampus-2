...
public class ValidationUtil {
     ...
     */
    public static String validateStudent(common.vo.StudentVO student) {
        if (student == null) {
            return "学生信息不能为空";
        }
        ...
        if (student.getEnrollmentYear() != null) {
            if (!isValidYear(student.getEnrollmentYear().toString())) {
                return "入学年份格式不正确，请输入1900-2100之间的有效年份";
            }
        }
     ...
     * @return 验证结果信息，null表示验证通过
     */
    public static String validateTeacher(common.vo.TeacherVO teacher) {
        if (teacher == null) {
            return "教师信息不能为空";
        }
        
        // 验证工号
        if (!isValidTeacherNo(teacher.getTeacherNo())) {
        ...
        // 验证手机号
        if (!isValidPhone(teacher.getPhone())) {
            return "手机号格式不正确，请输入有效的手机号码";
        }
