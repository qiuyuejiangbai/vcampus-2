-- 修复论坛数据脚本
-- 如果论坛数据缺失，执行此脚本来重新初始化

USE vcampus;

-- 检查当前数据状态
SELECT 'forum_threads' as table_name, COUNT(*) as count FROM forum_threads
UNION ALL
SELECT 'forum_posts', COUNT(*) FROM forum_posts
UNION ALL
SELECT 'forum_sections', COUNT(*) FROM forum_sections
UNION ALL
SELECT 'users', COUNT(*) FROM users;

-- 如果论坛板块为空，重新插入
INSERT IGNORE INTO forum_sections (name, description, sort_order, status) VALUES
('学术交流', '课程/学术问题讨论', 1, 1),
('校园生活', '生活/活动/社团', 2, 1),
('二手交易', '闲置物品交易', 3, 1),
('失物招领', '失物与招领信息', 4, 1),
...
-- 如果论坛主题为空，重新插入
INSERT IGNORE INTO forum_threads (title, content, author_id, section_id, category, is_essence, reply_count, view_count, like_count, favorite_count, is_pinned, is_locked, status, created_time, last_post_time) VALUES 
...
-- 如果论坛回复为空，重新插入
INSERT IGNORE INTO forum_posts (thread_id, content, author_id, parent_post_id, reply_level, reply_path, like_count, created_time) VALUES 
...
-- 插入标签数据
INSERT IGNORE INTO forum_tags (tag_name) VALUES ('Java'), ('学习'), ('公告'), ('活动'), ('图书');
...
-- 插入点赞数据
INSERT IGNORE INTO forum_likes (entity_type, entity_id, user_id, created_time) VALUES 
...
-- 插入收藏数据
INSERT IGNORE INTO forum_favorites (thread_id, user_id, created_time) VALUES 
(1, 5, '2024-03-06 10:40:00'),
(2, 6, '2024-03-08 10:20:00');

-- 插入公告数据
INSERT IGNORE INTO forum_announcements (title, content, is_pinned, status, created_time) VALUES 
('欢迎使用校园论坛系统！', '请遵守论坛规则，文明发言。', TRUE, 1, '2024-03-05 08:30:00'),
('期末考试安排已发布', '请进入教务系统查看详细安排。', TRUE, 1, '2024-03-08 09:00:00');

-- 最终检查数据状态
SELECT '修复后数据状态:' as status;
SELECT 'forum_threads' as table_name, COUNT(*) as count FROM forum_threads
UNION ALL
SELECT 'forum_posts', COUNT(*) FROM forum_posts
UNION ALL
...
UNION ALL
SELECT 'forum_favorites', COUNT(*) FROM forum_favorites;
