 ...
 * 用于显示所有选课记录信息
 */
public class EnrollmentTablePanel extends JPanel {
    ...
    private void setupMessageListener() {
        // 设置选课记录列表响应监听器
        serverConnection.setMessageListener(MessageType.GET_ALL_ENROLLMENTS_SUCCESS, message -> {
    ...
    private void initComponents() {
        ...
        // 设置列宽
        enrollmentTable.getColumnModel().getColumn(0).setPreferredWidth(100); // 学号
        enrollmentTable.getColumnModel().getColumn(1).setPreferredWidth(120); // 学生姓名
        enrollmentTable.getColumnModel().getColumn(2).setPreferredWidth(100); // 课程代码
        enrollmentTable.getColumnModel().getColumn(3).setPreferredWidth(200); // 课程名称
        enrollmentTable.getColumnModel().getColumn(4).setPreferredWidth(60);  // 学分
        enrollmentTable.getColumnModel().getColumn(5).setPreferredWidth(100); // 教师姓名
        enrollmentTable.getColumnModel().getColumn(6).setPreferredWidth(80);  // 学期
        enrollmentTable.getColumnModel().getColumn(7).setPreferredWidth(100); // 学年
        enrollmentTable.getColumnModel().getColumn(8).setPreferredWidth(150); // 选课时间
        enrollmentTable.getColumnModel().getColumn(9).setPreferredWidth(80);  // 状态
    ...
    private void loadEnrollmentData() {
        try {
            ...
            request.setType(MessageType.GET_ALL_ENROLLMENTS_REQUEST);
            request.setData(null);
            
            // 发送消息到服务器
            if (serverConnection.sendMessage(request)) {
                System.out.println("已发送获取选课记录列表请求");
                // 等待服务器响应，这里暂时清空表格
                enrollmentList.clear();
                updateTableData();
                System.out.println("等待服务器响应选课记录数据...");
            } else {
                System.err.println("发送获取选课记录数据请求失败");
                enrollmentList.clear();
                updateTableData();
            }
        } catch (Exception e) {
            System.err.println("加载选课记录数据时发生错误: " + e.getMessage());
            enrollmentList.clear();
            updateTableData();
        }
    }
